...
function* workerSaga(action) {
    try {
        ...
        var payload = {}
        var formatUrl = 'locationTypes-dropdown'
        yield request("get", action.payload, formatUrl).then(response => {
            payload = response;
        });
        yield put({ type: "LOCATION_TYPE_DROPDOWN_LIST", payload: payload })
        formatUrl = 'get-Locations/' + localStorage.getItem("companyId")
        console.log(formatUrl)
        yield request("get", payload, formatUrl).then(response => {
            payload = response;
        });
        yield put({ type: "LOCATIONS_LIST", payload: payload })
