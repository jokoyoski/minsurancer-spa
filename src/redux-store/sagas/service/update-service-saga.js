...
export default function* watcherUpdateServiceSaga() {
    yield takeEvery("UPDATE_SERVICE", workerSaga);
}
...
function* workerSaga(action) {
    try {
        ...
        let payload = {}
        var url = "api/SetUp/update-service"
        console.log(action.payload)
        yield request("put", action.payload, url).then(response => {
            payload = response;
        });
